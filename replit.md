...
- **CSV Data Append Logic Fix (2025-07-21)**: Fixed CSV upload merge logic to properly append missing data to existing products. Previously only updated fields when values were different; now also fills empty fields when new data is available. For example, if "Graffiti Blended Poly 12"x18"" exists without an Item Code, uploading CSV with Item Code will now add it to the existing product. Enhanced logging shows whether fields are being "added" (to empty fields) or "updated" (changed values). This ensures complete data population for all products
